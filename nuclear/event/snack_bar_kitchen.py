...
def event_a():
    print_message(text="냉장고에서 토마토 수프를 발견했습니다.")
    item.get(["토마토 수프"] * random.choice([1, 1, 1, 2, 2, 3, 4]))


def event_b():
    print_message(text="버려진 사탕을 발견했습니다.")
    print_message(text="또 그 사탕의 옆에 한 메모가 있었습니다.")
    print_message(text='"인터넷에서 사탕을 2Kg 사는 것은 나쁜 생각이다..."')
    item.get(["사탕"] * random.choice([3, 3, 3, 4, 5, 6]))


def event_c():
    print_message(text="수건을 발견했습니다.")
    item.get(["수건"])


def event_d():
    print_message(text="당신보다 빨리 이 식당에 온 사람이 있었나 봅니다.")
    print_message(text="수납장에는 아무것도 없었습니다.")


def event_e():
    print_message(text="환풍기는 고장이 난 듯 아무런 반응이 보이지 않습니다.")
...
def start():
    ...
    print_message(text="그때 조그만한 불씨가 당신을 위협합니다.")
